    ...
    if ($_POST['password'] === $_POST['confirm']) {
        if (strlen($_POST['password']) > 5) {
            ...
            if ($user === false) {
                ...
                $query = 'INSERT INTO php_project.users SET email= ? ,first_name= ? ,last_name= ? ,password= ? , create_at=NOW();';
                $statement = $pdo->prepare($query);
            ...
            $error = 'تعداد حروف رمز عبور باید بیشتر از 5 حرف باشد';
        }

    } else {
        $error = 'رمز عبور وارد شده با تاییدیه رمز عبور مطابقت ندارد';
    }
} else {
    if (!empty($_POST)) {
        $error = 'همه فیلد ها اجباریست';
    }
}
